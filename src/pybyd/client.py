...
class BydClient:
    ...
    async def stop_charging(
        ...
        *,
        ...
        Mirror of :meth:`start_charging` using ``status: "0"`` on the
        same ``/control/smartCharge/changeChargeStatue`` endpoint.  Same
        gating (functionNo ``1012``).  Cloud returns res=3 when the
        vehicle isn't currently charging.
        """
        capabilities = await self.get_vehicle_capabilities(vin)
        gate = evaluate_command_gate(RemoteCommand.STOP_CHARGE, capabilities)
        if not gate.supported:
            raise BydEndpointNotSupportedError(
        ...
        async def _call() -> ChargeChangeResult:
            ...
            if res == 3:
                raise BydRemoteControlError(
                    f"smartCharge change_charge_status rejected by cloud (res=3, "
                    f"message={result.message!r}) — vehicle is likely not charging",
                    code="3",
                    endpoint="/control/smartCharge/changeResult",
                )
            raise BydRemoteControlError(
